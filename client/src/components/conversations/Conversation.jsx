...
export default function Conversation({
  ...
  const friendId = conversation.members
    ? conversation.members.find((m) => m !== currentUser.others._id)
    : "";
  ...
  useEffect(() => {
    const getLastMessage = async () => {
      try {
        const res = await ChatAppApi.get(
          `/messages/lastOne/${conversation._id}`
        );
        ...
        <>
          <div className="conversation">
            {conversation.members.length >= 2 ? (
              <AvatarGroup max={conversation.members.length + 1}>
                <Avatar
            ...
            <div>
              <span className="conversationName" style={convoDesc}>
                {conversation.name}
              </span>
              <p className="latestConvoMsg" style={convoDesc}>
                {conversation.messages.length > 0
                  ? conversation.messages[conversation.messages.length - 1]
                      .sender +
                    " : " +
                    conversation.messages[conversation.messages.length - 1].msg
                  : ""}
              </p>
            </div>
