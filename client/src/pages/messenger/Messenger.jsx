...
export default function Messenger() {
  ...
  useEffect(() => {
    ...
    const handleChatUser = async () => {
      ...
      const sender = currentChat.members.filter(
        (member) => user.others._id !== member
      );
      await ChatAppApi.get("/users?userId=" + sender)
        .then((res) => {
  ...
  const handleSearch = async (e, isRoom) => {
    if (isRoom) {
      ...
      if (e.target.value.length > 0) {
        const rooms = await ChatAppApi.post("/rooms/search/", {
    ...
    if (e.target.value.length > 0) {
      const senders = await ChatAppApi.post("/users/search/", {
        search: e.target.value,
      });
      if (senders.data.length > 0) {
        const chats = senders.data.filter(
          (sender) => sender._id !== user.others._id
        );
  ...
  const handleChatBox = async (receiver) => {
    ...
    const res = await ChatAppApi.post("/conversations/", {
      senderId: user.others._id,
      ...
      <div className="messenger" style={messenger}>
        ...
        <div className="chatMenu">
          <div className="chatMenuWrapper">
            <Tabs>
              <TabList>
                <Tab style={chatMsgInput}>
                  <h3 className="ChatContainerTitle">
              ...
              <TabPanel>
                ...
                {searchUsers.length > 0 && searchValue.length > 0 ? (
                  searchUsers.map((searchUser, i) => (
                    <div key={i} onClick={() => handleChatBox(searchUser)}>
                      <Conversation
                ...
                ) : searchUsers.length < 1 && searchValue.length > 0 ? (
                  <div className="noConversation" style={convoDesc}>
                  ...
                  <div className="noConversation" style={convoDesc}>
                    <i>No Chat yet! find people to join convo</i>
                  </div>
                )}
              </TabPanel>
              <TabPanel>
                ...
                {searchRooms.length > 0 && roomSearchValue.length > 0 ? (
                  searchRooms.map((searchRoom, i) => (
                    <div key={i} onClick={() => handleChatBox(searchRoom)}>
                      <Conversation
                        conversation={searchRoom}
                        currentUser={user}
                        roomsLength={rooms.length}
                      />
                    </div>
                  ))
                ) : searchRooms.length < 1 && roomSearchValue.length > 0 ? (
                  <div className="noConversation" style={convoDesc}>
                    <span>No result found</span>
                  </div>
                ) : (
